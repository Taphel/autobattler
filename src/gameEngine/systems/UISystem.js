    ...
    update(gameState, entities, components, spriteSize, deltaTime, unitPool, boardSize) {
        ...
        entities.forEach(entity => {
            if (unitOverlayUI.has(entity)) {
                overlayIds.push(entity);
            }
        })

        overlayIds.forEach((overlay, index) => {
            const overlayComponent = unitOverlayUI.get(overlay);
            overlayComponent.setTargetId(unitIds[index]);
        })

        switch (gameState) {
            case GameState.idle:
                this.#updateBattleUI(entities, components, spriteSize, deltaTime);
            case GameState.battle:
                this.#updateBattleUI(entities, components, spriteSize, deltaTime);
        }
    }

    #updateBattleUI(entities, components, spriteSize, deltaTime) {
